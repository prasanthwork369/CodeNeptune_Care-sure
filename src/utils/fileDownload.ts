...
export const downloadFile = async (url: string, fileName?: string): Promise<void> => {
    try {
        ...
        const mimeType = originalExtension === 'pdf' ? 'application/pdf' : `image/${originalExtension === 'png' ? 'png' : 'jpeg'}`;
        ...
        Alert.alert('Download Error', error.message || 'Failed to download prescription. Please try again.');
    }
};
...
const persistLocalFile = async (
    ...
    if (Platform.OS === 'android') {
        ...
        try {
            await ReactNativeBlobUtil.fs.scanFile([{ path: destPath, mime: mimeType }]);
        } catch (scanError) {
        ...
        try {
            ...
            Alert.alert('Success', `${itemTypeLabel} saved to Downloads folder: ${finalFileName}`);
        }
    } else {
...
export const downloadLocalAsset = async (assetModule: number, fileName: string): Promise<void> => {
    try {
        ...
        Alert.alert('Download Error', error.message || 'Failed to save file. Please try again.');
    }
};
 ...
 */
export const downloadLocalFile = async (uri: string, fileName: string): Promise<void> => {
    try {
        const ReactNativeBlobUtil = getBlobUtil();
        if (!ReactNativeBlobUtil) {
            Alert.alert('Not Available', 'File downloads require the development build and are not available in Expo Go.');
