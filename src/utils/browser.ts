...
function resolveUrl(url: string): string {
    ...
    try {
        const links = queryClient.getQueryData<MobileAppLinks>(['mobile-app-links']);
        if (links) {
            if (url.includes('/terms') && links.termsLink) {
                return links.termsLink;
            }
            if ((url.includes('/privacy-policy') || url.includes('/privacy')) && links.privacyLink) {
                return links.privacyLink;
            }
            if ((url.includes('/refund-policy') || url.includes('/refund')) && links.refundLink) {
 ...
 */
export async function openInAppBrowser(url: string, toolbarColor: string = '#1A1A1A'): Promise<void> {
