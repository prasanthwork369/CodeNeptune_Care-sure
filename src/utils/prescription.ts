...
export async function validatePrescriptionFile(
    ...
    try {
        ...
        if (info.exists && 'size' in info && typeof info.size === 'number' && info.size > 0) {
            size = info.size;
            if (__DEV__) console.log(`[Prescription] FS-verified size for "${name}": ${size} bytes`);
        } else {
            if (__DEV__) console.warn(`[Prescription] FS size unavailable for "${name}", using picker size: ${size}`);
        }
    } catch (e) {
    ...
    if (size <= 0) {
        cleanup();
        onError?.('Upload Restriction', `Could not determine the size of "${name}".`);
        return null;
    }
