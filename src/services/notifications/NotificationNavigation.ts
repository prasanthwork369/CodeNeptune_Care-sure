...
export const NotificationNavigation = {
  ...
  handleTap: (payload: NotificationPayload, tapId?: string) => {
    ...
    if (!payload.type) {
      if (__DEV__)
        console.warn(
          "[NotificationNavigation] Exiting handleTap: No valid notification type found in payload.",
        );
      return;
    }
  ...
  executeNavigation: (payload: NotificationPayload) => {
    ...
    switch (type) {
      ...
      default:
        if (__DEV__)
          console.warn(
            `[NotificationNavigation] Unhandled or unknown notification type: ${type}. Falling back to Notifications.`,
          );
        router.push("/notifications");
        break;
    }
