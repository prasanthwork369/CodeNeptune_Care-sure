...
export const storageApi = {
    ...
    upload: async (file: UploadFile, folder: string): Promise<UploadedImage> => {
        ...
        if (__DEV__) console.log('[storageApi.upload] raw response.data:', JSON.stringify(response.data));
        return {
            url: data.url,
    ...
    delete: async (path: string): Promise<void> => {
        await apiClient.delete(API_ENDPOINTS.STORAGE_DELETE, { data: { key: path } });
    },
