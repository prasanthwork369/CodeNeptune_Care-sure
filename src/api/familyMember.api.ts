...
export const familyMemberApi = {
    ...
    addMember: async (payload: FamilyMemberInput): Promise<FamilyMember> => {
        const { data } = await apiClient.post(API_ENDPOINTS.FAMILY_MEMBERS, payload);
        return data.data as FamilyMember;
    },

    updateMember: async (memberId: string, payload: Partial<FamilyMemberInput>): Promise<FamilyMember> => {
        const { data } = await apiClient.patch(API_ENDPOINTS.FAMILY_MEMBER_BY_ID(memberId), payload);
        return data.data as FamilyMember;
    },
