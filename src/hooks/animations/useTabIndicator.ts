...
export const useTabIndicator = (activeId: string) => {
    ...
    const [tabLayouts, setTabLayouts] = useState<Record<string, TabMeasuredLayout>>({});
    ...
    useEffect(() => {
        if (activeId && tabLayouts[activeId]) {
            ...
            indicatorX.value = withTiming(targetX, { duration: 220, easing: Easing.out(Easing.quad) });
            indicatorWidth.value = withTiming(STATIC_WIDTH, { duration: 220, easing: Easing.out(Easing.quad) });
