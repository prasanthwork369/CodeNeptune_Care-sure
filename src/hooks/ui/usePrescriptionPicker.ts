...
export function usePrescriptionPicker(
    ...
    const navigate = (files: PrescriptionItem[]) => {
        ...
        const existingKeys = new Set(currentItems.map(it => `${it.name}-${it.size}-${it.type}`));
    ...
    const pickImages = () => { onClose(); setTimeout(selectImages, 400); };
    const pickPdf = () => { onClose(); setTimeout(selectPdfs, 400); };
