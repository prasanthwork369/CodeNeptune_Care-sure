...
export const useTabIndicator = (activeId: string) => {
    ...
    useEffect(() => {
        if (activeId && tabLayouts[activeId]) {
            ...
            indicatorX.value = withTiming(x, { duration: 220, easing: Easing.out(Easing.quad) });
            indicatorWidth.value = withTiming(width, { duration: 220, easing: Easing.out(Easing.quad) });
