...
import { AppState, AppStateStatus, View, useWindowDimensions } from 'react-native';
...
const sameSlug = (a?: string, b?: string) =>
    !!a && !!b && a.toLowerCase().replace(/[-_\s]/g, '') === b.toLowerCase().replace(/[-_\s]/g, '');

const findCardBySlug = (link: string, categories?: CategoryCard[]): CategoryCard | undefined => {
    ...
    return categories.find((c) => sameSlug(c.slug, slug) || sameSlug(c.familySlug, slug));
};
        ...
        <View>
            ...
            {canLoop && (
                <View className="flex-row justify-center items-center mt-2 gap-x-1.5">
                    {banners.map((_, i) => (
                        <CarouselDot key={i} index={i} progress={progressShared} total={banners.length} />
                    ))}
                </View>
            )}
        </View>
