...
export const HomeSearchCycler: React.FC = () => {
    ...
    const animate = () => {
        ...
        if (activeSlot.current === 'a') {
            ...
            aOp.value = withTiming(0, { duration: ANIM_MS * 0.65, easing: Easing.in(Easing.cubic) });
            bY.value  = withTiming(0, { duration: ANIM_MS, easing: EASE }, (done) => {
                if (done) runOnJS(onDone)();
            });
            bOp.value = withDelay(
                ANIM_MS * 0.25,
                withTiming(1, { duration: ANIM_MS * 0.75, easing: Easing.out(Easing.cubic) }),
            );
        } else {
            bY.value  = withTiming(-SLOT_H, { duration: ANIM_MS, easing: EASE });
            bOp.value = withTiming(0, { duration: ANIM_MS * 0.65, easing: Easing.in(Easing.cubic) });
