...
export const ProductDetailsLayout: React.FC = () => {
    ...
    useEffect(() => {
        if (variants.length > 0 && !selectedVariantId) {
            const inCart = variants.find(v =>
                cartItems.some(i =>
                    i.medicineId === v.id ||
                    (i.metadata?.packSize === v.packSize && i.metadata?.unit === v.unit),
                ),
            );
    ...
    const effectiveDiscountPct = selectedVariant
        ? (selectedVariant.discountPercentage > 0 ? selectedVariant.discountPercentage : (product?.savingsPercent ?? 0))
        : 0;
    const variantSellingPrice = selectedVariant
        ? (effectiveDiscountPct > 0
            ? parseFloat((selectedVariant.price * (1 - effectiveDiscountPct / 100)).toFixed(2))
    ...
    const { containerStyle, contentStyle, backdropStyle, screenStyle, handleBack } =
    ...
    return (
        <Animated.View style={[{ flex: 1, backgroundColor: 'transparent' }, screenStyle]}>

            {/* Dims home screen during expansion */}
            <Animated.View
                style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,0.15)' }, backdropStyle]}
                pointerEvents="none"
            />

            {/* Expanding container — grows from card rect to full screen, clips content */}
            <Animated.View style={[{ backgroundColor: '#F9FAFB' }, containerStyle]}>
                    ...
                    <View style={{ flex: 1, backgroundColor: '#F9FAFB' }}>
                        ...
                        {isLoading ? (
                            ...
                            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 32 }}>
                                <Text style={{ fontSize: 16, fontWeight: '600', color: '#333232', textAlign: 'center' }}>
                                    Product not found
                                </Text>
                            </View>
                        ) : (
                            ...
                            >
                                {saltComposition && <SaltCompositionBanner composition={saltComposition} />}
