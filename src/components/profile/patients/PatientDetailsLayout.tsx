...
const PatientCard = ({ item, onEdit, onDelete, deleting, isLast }: { item: FamilyMember; onEdit: (p: FamilyMember) => void; onDelete: (id: string) => void; deleting: string | null; isLast: boolean }) => {
    ...
    return (
        <View>
            ...
            >
                <View className="items-center justify-center rounded-full mr-3" style={[s.patientAvatar, { width: 48, height: 48, backgroundColor: '#E3EEEE' }]}>
                    <icons.user_active width={22} height={22} />
                </View>
                <View className="flex-1">
                    <Text style={s.patientName} className="font-inter-bold text-brand-text mb-1.5" numberOfLines={1}>{item.name}</Text>
                    <View className="flex-row items-center gap-x-2">
                        ...
                        <View className="px-2 py-0.5 rounded-full border border-[#DDE1E5]" style={{ backgroundColor: '#FAFAFA' }}>
                            <Text style={s.patientTag} className="font-inter-medium text-[#222222]">{item.relationship}</Text>
                        </View>
                    </View>
                </View>
                <View style={{ width: 1, height: 16, backgroundColor: '#E1E5E8', marginRight: 8 }} />
                <Touchable onPress={() => onEdit(item)} activeOpacity={0.6} className="mr-3" hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                    <Text style={s.patientDetail} className="font-inter-semibold text-[#6A6A6A]">Edit</Text>
                </Touchable>
    ...
    return (
        <View className="flex-1 bg-[#F5F6FB]">
            ...
            <View className="px-4 pt-5 pb-1 bg-[#F5F6FB]">
                <Touchable activeOpacity={0.85} onPress={() => router.push('/profile/patients/add')} className="bg-[#0F7635] flex-row items-center justify-center" style={{ height: 50, borderRadius: 12 }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 16, paddingBottom: 40, flexGrow: 1 }} className="flex-1">
                    ...
                    >
                        {members.map((item, index) => (
                            <PatientCard
                                key={item.id}
                                item={item}
                                onEdit={(p) => router.push({ pathname: '/profile/patients/add', params: { id: p.id } })}
