import React from 'react';
import { View, Text } from 'react-native';
import { cartStyles as s } from '../cart.styles';
import { LinearGradient } from 'expo-linear-gradient';
import { CartSavingsBannerProps } from '@/src/types/cart';
import { Image } from 'expo-image';
import { HOME_IMAGES } from '@/src/constants/images';

export const CartSavingsBanner: React.FC<CartSavingsBannerProps> = ({ totalSavings }) => {
    if (totalSavings <= 0) return null;

    return (

         <LinearGradient
            colors={["#D0EBFE", "#D7FFEA"]}
            start={{ x: 0, y: 0.5 }}
            end={{ x: 1, y: 0.5 }}
            style={{
              flexDirection: "row",
              alignItems: "center",
              paddingHorizontal: 14,
              paddingVertical: 12,
              gap: 10,
              marginHorizontal: 16, marginTop: 14, borderRadius: 12, borderWidth: 1, borderColor: '#919EAB33' 
            }}
          >

            <Image
              source={HOME_IMAGES.discountTag}
              style={{ width: 32, height: 32 }}
              resizeMode="contain"
            />
            <Text
              style={{ fontSize: 14, fontFamily: "Inter-SemiBold", color: "#0A0A0A" }}
            >
              {"You saved  "}
                                 <Text className="font-inter-extrabold text-[#0A0A0A]">₹{parseFloat(totalSavings.toFixed(2))}</Text> on this Order
              {" on this Order"}
            </Text>
            </LinearGradient>
      
    );
};
