        ...
        <View style={{ marginHorizontal: 16, marginTop: 12, borderRadius: 12, borderWidth: 1, borderColor: '#919EAB33', backgroundColor: 'white', overflow: 'hidden' }}>
            <LinearGradient colors={['#FBFEFC', '#EBFAF0']} start={{ x: 0.5, y: 1 }} end={{ x: 0.5, y: 0 }}>
                <View className="px-4 py-3 flex-row items-center">
                    ...
                    <Text className="flex-1 font-inter-bold text-brand-text ml-2" style={{ fontSize: moderateScale(14) }}>Saving this order</Text>
                    <LinearGradient colors={['#68D36C', '#329939']} start={{ x: 0.5, y: 0 }} end={{ x: 0.5, y: 1 }} style={{ borderRadius: 6 }}>
                        <View className="px-2.5 py-1">
                            <Text className="font-inter-bold text-white" style={{ fontSize: moderateScale(12) }}>₹{parseFloat(totalSavings.toFixed(2))}</Text>
                        </View>
                    </LinearGradient>
                </View>
            </LinearGradient>
            ...
            <View className="px-4 py-2">
                {rows.map(row => (
                    <View key={row.label} className="flex-row items-center justify-between py-2">
                        <Text className={`font-inter-medium ${row.value > 0 ? 'text-[#6A6A6A]' : 'text-[#C0C0C0]'}`} style={{ fontSize: moderateScale(13) }}>{row.label}</Text>
                        <Text className={`font-inter-semibold ${row.value > 0 ? 'text-[#0F7635]' : 'text-[#C0C0C0]'}`} style={{ fontSize: moderateScale(13) }}>
