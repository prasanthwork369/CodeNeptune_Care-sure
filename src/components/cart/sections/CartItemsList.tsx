                    ...
                    <View className="flex-row py-4">
                        ...
                        >
                            <View style={[s.itemImgBox, { backgroundColor: 'white', borderWidth: 1, borderColor: '#919EAB33', borderRadius: 10, alignItems: 'center', justifyContent: 'center', position: 'relative' }]}>
                                ...
                                {line.rx && (
                                    ...
                                    >
                                        <Text style={s.itemRxText} className="font-inter-semibold text-brand-text leading-none">Rx</Text>
                                    </View>
                                )}
                            </View>
                            <View className="flex-1 ml-3">
                                <Text numberOfLines={2} style={s.itemTitle} className="font-inter-bold text-brand-text">{line.name}</Text>
                                <Text style={s.itemSub} className="font-inter-medium text-brand-subtext mt-1">{[line.brand, line.pack].filter(Boolean).join(' • ')}</Text>
                                {!!line.discount && (
                                    <Text style={s.itemDiscount} className="font-inter-bold text-brand-primary mt-1.5">{line.discount}</Text>
                                )}
                            </View>
                        </Pressable>
                        <View className="items-end justify-between ml-2">
                            ...
                            <View className="flex-row items-baseline mt-3">
                                ...
                                <Text style={s.itemPrice} className="font-inter-bold text-brand-text">₹{Number(line.price * line.qty).toFixed(2)}</Text>
                            </View>
                        </View>
                    </View>
                </View>
