...
export const UploadPrescriptionSheet: React.FC<UploadPrescriptionSheetProps> = ({
    isVisible,
    onClose,
    toPay,
    patientMemberId,
    ...
    const { pickImages, pickPdf, takePhoto } = usePrescriptionPicker(onClose, toPay, patientMemberId, (title, message, onDismiss) => setInfoModal({ title, message, onDismiss }));
                ...
                >
                    ...
                    <View className="flex-row gap-3">
                        ...
                        >
                            ...
                            <Text className="text-[13px] font-inter-medium text-brand-text">Upload Images</Text>
                        </Touchable>

                        <Touchable
                            activeOpacity={0.85}
                            onPress={() => { onClose(); setTimeout(handleTakePhoto, 400); }}
                            style={{ borderColor: '#919EAB33' }}
                            className="flex-1 items-center border rounded-2xl py-5 bg-white"
                        >
                            <View
                            ...
                            <Text className="text-[13px] font-inter-medium text-brand-text">Take a Photo</Text>
                        </Touchable>
                        ...
                        >
                            ...
                            <Text className="text-[13px] font-inter-medium text-brand-text">Upload PDF</Text>
                        </Touchable>
                    </View>
                    ...
                    <Touchable
                        onPress={() => {
                            ...
                            router.push({ pathname: '/prescription-history', params: toPay ? { toPay, source: 'cart' } : {} });
                        }}
                        activeOpacity={0.85}
                        style={{
                    ...
                    >
                        ...
                        <View className="flex-1 ml-3">
                            ...
                            <Text style={{ color: colors.primary }} className="self-start text-[10px] bg-[#F3FAF7] px-3 py-1 rounded-lg font-inter-semibold tracking-wider mt-1">
                                FASTER VERIFICATION
                            </Text>
                        </View>
                    ...
                    <Touchable
                        activeOpacity={0.8}
                        onPress={() => setShowBeforeUpload((v) => !v)}
                        className="flex-row items-center mt-5"
                    >
                        <Text className="text-[14px] font-inter-semibold text-brand-text">Before You Upload</Text>
                        <View
                            style={{
                    ...
                    {showBeforeUpload && (
                        ...
                        >
                                ...
                                <View className="flex-1 ml-4">
                                    ...
                                    {VALID_ITEMS.map((label, idx) => (
                                        <View key={label} className="flex-row items-center mb-1.5">
                                            ...
                                            >
                                                <Text className="text-[10px] font-inter-bold text-white">{idx + 1}</Text>
                                            </View>
                                            <Text className="text-[12px] font-inter-medium text-brand-text">{label}</Text>
                                        </View>
                                    ))}
                                </View>
                            </View>
                            ...
                            <View
                                className="mt-3 mb-3"
                                style={{ borderTopWidth: 1, borderColor: '#0F763544', borderStyle: 'dotted' }}
                            />
