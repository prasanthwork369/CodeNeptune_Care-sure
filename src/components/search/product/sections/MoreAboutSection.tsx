...
export const MoreAboutSection: React.FC<MoreAboutSectionProps> = ({ medicineName, mobileAdditionalData }) => {
    ...
    const handleTabPress = (tabId: string) => {
        ...
        if (layout) {
            indicatorX.value = withTiming(layout.x, { duration: 280, easing: Easing.bezier(0.25, 1, 0.5, 1) });
            indicatorWidth.value = withTiming(layout.width, { duration: 280, easing: Easing.bezier(0.25, 1, 0.5, 1) });
            const screenWidth = Dimensions.get('window').width;
            setTimeout(() => {
                tabsScrollRef.current?.scrollTo({ x: Math.max(0, layout.x - (screenWidth - layout.width) / 2), animated: true });
            }, 50);
        }
        contentOpacity.value = withTiming(0, { duration: 120, easing: Easing.out(Easing.quad) }, (finished) => {
            if (finished) {
    ...
    const toggleExpand = (id: string) => {
        ...
        if (isExpanding && cardXOffsets.current[id] !== undefined) {
            setTimeout(() => {
                ...
                const centeredX = Math.max(0, cardXOffsets.current[id] - (screenWidth - expandedWidth) / 2);
    ...
    return (
        <View className="mx-4 mb-6 mt-2">
            ...
            <Animated.View className="rounded-[12px] bg-white" layout={LinearTransition.duration(260).easing(Easing.out(Easing.quad))}>
                <ScrollView
                    ref={tabsScrollRef}
                    horizontal
                ...
                >
                    <View style={{ position: 'relative' }}>
                                ...
                                <Touchable
                                    ...
                                    onLayout={(e) => {
                                        ...
                                        if (tab.id === activeTab && indicatorWidth.value === 0) {
                                            ...
                                            const screenWidth = Dimensions.get('window').width;
                                            setTimeout(() => {
                                                tabsScrollRef.current?.scrollTo({ x: Math.max(0, x - (screenWidth - width) / 2), animated: true });
                                            }, 100);
                                        }
                                    }}
                                >
                                    <Text className={`text-[14px] font-inter-medium ${activeTab === tab.id ? 'text-brand-primary' : 'text-brand-subtext'}`}>
                                        {tab.label}
                                    </Text>
                                </Touchable>
                            ))}
                        ...
                        <Animated.View
                            style={[
                                { position: 'absolute', bottom: 0, height: 3, borderRadius: 2, backgroundColor: '#0F7635' },
                                indicatorStyle,
                            ]}
                        />
                    </View>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <View className="self-start px-2 py-1 rounded-[4px] mb-2" style={{ backgroundColor: item.statusBg }}>
                                                <Text className="text-[10px] font-inter-semibold" numberOfLines={1} adjustsFontSizeToFit style={{ color: item.statusColor }}>{item.status}</Text>
                                            </View>
                                            <Text className="text-[12px] font-inter-medium text-brand-subtext leading-[18px]">
                                                ...
                                                {item.expandable && (
                                                    <Text onPress={() => toggleExpand(item.id)} className="text-[#0F7635] font-inter-bold text-sm">
