    ...
    return (
        <View className="bg-transparent pt-4 pb-4">
            ...
            <View className="px-5">
                ...
                <Text className="font-inter-bold text-[#111827] mb-3 leading-[28px]" style={{ fontSize: moderateScale(20) }}>
                    {product.name}
                </Text>
                ...
                <View className="flex-row items-baseline gap-x-1.5 mb-1">
                    ...
                    {!!product.originalPrice && product.originalPrice > product.price && (
                        <>
                            <Text className="font-inter-medium text-brand-subtext ml-1" style={{ fontSize: moderateScale(13) }}>MRP</Text>
            ...
            <View className="px-5 flex-row items-center justify-between">
                <View>
                    <View className="flex-row items-baseline gap-x-1.5">
                        <Text className="font-inter-bold text-[#111827]" style={{ fontSize: moderateScale(18) }}>
                            ₹{Number(product.price).toFixed(2)}
                        </Text>
                        {!!product.originalPrice && product.originalPrice > product.price && (
                        ...
                        {!!product.savingsPercent && (
                            <View className="bg-[#E8F0FE] px-1.5 py-0.5 rounded">
                                <Text className="font-inter-bold text-[#1A73E8]" style={{ fontSize: moderateScale(10) }}>
                                    {product.savingsPercent}% off
                                </Text>
                            </View>
                        )}
                    </View>
                    <Text className="font-inter-medium text-brand-subtext mt-0.5" style={{ fontSize: moderateScale(11) }}>
                ...
                {count === 0 ? (
                    ...
                    >
                        <Text className="font-inter-bold" style={{ color: '#0F7635', fontSize: moderateScale(14) }}>
                    ...
                    <View className="flex-row items-center justify-between rounded-[10px] w-[120px]" style={{ height: CART_BUTTON_HEIGHT, backgroundColor: '#0F7635' }}>
                        ...
                        >
                            <Text className="font-inter-medium text-white leading-none" style={{ fontSize: moderateScale(20) }}>−</Text>
                        </Touchable>
                        <View style={{ width: 32, height: 24, alignItems: 'center', justifyContent: 'center' }}>
                            {isPending ? (
                                ...
                                <Animated.Text
                                    style={{ transform: [{ translateY: slideAnim }], opacity: opacityAnim, fontSize: moderateScale(14) }}
