        ...
        <View className="bg-white rounded-2xl border border-[#919EAB33] p-4 mb-3">
            <View className="flex-row items-center">
                ...
                <View className="flex-1">
                    <Text style={s.cardTitle} className="font-inter-semibold text-brand-text mb-1">{title}</Text>
                    <Text style={s.cardDate} className="font-inter-medium text-[#6A6A6A]">{date}</Text>
                </View>

                {/* Right Amount Content */}
                <View className="flex-row items-center">
                    {amountImage && (
                        <Image source={amountImage} style={s.cardAmtImg} contentFit="contain" />
                    )}
                    <Text style={s.cardAmount} className={`font-inter-bold text-[#0F7635] ${amountImage ? 'ml-1.5' : ''}`}>
                        {amount}
                    </Text>
                </View>
            </View>
