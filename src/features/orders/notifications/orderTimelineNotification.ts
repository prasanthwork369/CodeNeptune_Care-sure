...
import { NOTIFICATION_ACTIONS } from "../../../services/notifications/notificationActions";
...
export const OrderTimelineHandler: NotificationHandler = {
  display: async (data: NotificationData): Promise<boolean> => {
    try {
      await notifeeService.displayBranded({
        data,
      });
      return true;
    } catch {
